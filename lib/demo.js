...
(function($) {
  var currentSideMenuItem, hostname, lastSideMenuY, piwikTracker, s, sideMenu, sideMenuItems, sideMenuMinY, t;
  sideMenu = $('.side-menu');
  ...
  */
  ...
  $(window).scroll(function(e) {
    ...
    sideMenuItems.each(function() {
      var $sideMenuItem, itemY;
      $sideMenuItem = $(this);
      $sideMenuItem.removeClass('current');
      itemY = $($sideMenuItem.attr('href')).position().top;
      if (itemY < scrollY) {
        return $currentItem = $sideMenuItem;
      } else {
        return false;
      }
    });
    return $currentItem != null ? $currentItem.addClass('current') : void 0;
  }).trigger('scroll');
