...
(function(factory) {
  ...
  hideSmallipop = function(e) {
    var direction, ignorePopupClick, ignoreTriggerClick, popup, popupData, popupId, shownId, target, trigger, triggerIsTarget, triggerOptions, xDistance, yDistance, _base, _ref, _ref1, _results;
    ...
    for (popupId in instances) {
      ...
      if (popupData[dataIsTour] && !popup.is(target) && !(triggerIsTarget && popup.is(trigger.data(classBase).popupInstance))) {
        continue;
      }
      trigger = getTrigger(shownId);
      triggerOptions = ((_ref = trigger.data(classBase)) != null ? _ref.options : void 0) || sip.defaults;
      if (popupData[dataIsTour]) {
        currentTour = null;
        if ((_ref1 = trigger.data(classBase)) != null) {
          if (typeof (_base = _ref1.options).onTourClose === "function") {
            _base.onTourClose();
          }
        }
  ...
  return $.fn.smallipop = function(options, hint) {
    ...
    return this.each(function() {
      var $objInfo, $self, isFormElement, newId, objHint, option, optionName, tagName, tourTitle, triggerData, triggerEvents, triggerOptions, triggerPopupInstance, type, value;
      $self = $(this);
      ...
      if (objHint && !$self.hasClass(classInitialized)) {
        ...
        if (triggerOptions.tourIndex) {
          ...
          if (triggerOptions.triggerOnClick || (triggerOptions.touchSupport && touchEnabled)) {
