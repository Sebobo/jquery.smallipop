  ...
  $.smallipop = sip = {
    ...
    _refreshPosition: function() {
      ...
      for (popupId in _ref) {
        ...
        if (yOverflow > 0) {
          popupOffsetTop = Math.max(popupOffsetTop - yOverflow, offset.top + yOffset + windowPadding);
        }
        xOverflow = popupOffsetLeft + popupW + windowPadding + xDistance + xOffset - winScrollLeft - winWidth;
        if (xOverflow > 0) {
          popupOffsetLeft = Math.max(popupOffsetLeft - xOverflow, offset.left + xOffset + windowPadding);
        }
