...
(function($) {
  ...
  $.smallipop = sip = {
    ...
    _refreshPosition: function() {
      var isFixed, offset, opacity, options, popup, popupCenter, popupData, popupDistanceBottom, popupDistanceLeft, popupDistanceRight, popupDistanceTop, popupH, popupId, popupOffsetLeft, popupOffsetTop, popupW, popupY, preferredPosition, selfHeight, selfWidth, selfY, shownId, trigger, win, winHeight, winScrollLeft, winScrollTop, winWidth, windowPadding, xDistance, xOverflow, yDistance, yOffset, yOverflow, _ref, _results;
      _ref = sip.instances;
      _results = [];
      for (popupId in _ref) {
        popup = _ref[popupId];
    ...
    _tourShow: function(title, index) {
      ...
      content = $.trim("        <div class=\"smallipop-tour-content\">" + triggerData.hint + "</div>        " + closeIcon + "        <div class=\"smallipop-tour-footer\">          <div class=\"smallipop-tour-progress\">            " + (index + 1) + " " + sip.labels.of + " " + currentTourItems.length + "          </div>          " + prevButton + "          " + nextButton + "          " + closeButton + "          <br style=\"clear:both;\"/>        </div>");
