  ...
  $.smallipop = sip = {
    ...
    _hideSmallipop: function(e) {
      ...
      for (popupId in _ref) {
        ...
        if (popupData.isTour) {
          ...
          this._getTourOverlay(triggerOptions).fadeOut(triggerOptions.tourHightlightFadeDuration);
        }
    ...
    */
    ...
    _showPopup: function(trigger, content) {
      var idx, lastTrigger, lastTriggerOpt, popup, popupContent, popupPosition, shownId, tour, tourOverlay, tourTrigger, triggerData, triggerOptions;
      if (content == null) {
        content = '';
      }
      ...
      if (triggerOptions.tourHighlight) {
        for (tour in sip.tours) {
          for (idx in sip.tours[tour]) {
